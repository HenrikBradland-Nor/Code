...
clientID=sim_code.simxStart('127.0.0.1', 19999, True, True, 5000, 5) # Connect to CoppeliaSim
if clientID!=-1:
    print ('Connected to remote API server')

    # 1. First send a command to display a specific message in a dialog box:
    emptyBuff = bytearray()
    res,retInts,retFloats,retStrings,retBuffer=sim_code.simxCallScriptFunction(clientID, 'remoteApiCommandServer', sim_code.sim_scripttype_childscript, 'displayText_function', [], [], ['Hello world!'], emptyBuff, sim_code.simx_opmode_blocking)
    if res==sim_code.simx_return_ok:
        print ('Return string: ',retStrings[0]) # display the reply from CoppeliaSim (in this case, just a string)
    else:
        print ('Remote function call failed')

    # 2. Now create a dummy object at coordinate 0.1,0.2,0.3 with name 'MyDummyName':
    res,retInts,retFloats,retStrings,retBuffer=sim_code.simxCallScriptFunction(clientID, 'remoteApiCommandServer', sim_code.sim_scripttype_childscript, 'createDummy_function', [], [0.1, 0.2, 0.3], ['MyDummyName'], emptyBuff, sim_code.simx_opmode_blocking)
    if res==sim_code.simx_return_ok:
    ...
    "return 'done'"
    res,retInts,retFloats,retStrings,retBuffer=sim_code.simxCallScriptFunction(clientID, "remoteApiCommandServer", sim_code.sim_scripttype_childscript, 'executeCode_function', [], [], [code], emptyBuff, sim_code.simx_opmode_blocking)
    if res==sim_code.simx_return_ok:
