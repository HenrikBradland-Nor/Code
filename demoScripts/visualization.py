...
def csimPrint(client_id, message):
    ...
    """
    sim_code.simxAddStatusbarMessage(client_id, message, sim_code.simx_opmode_oneshot)
    print(message)


def create_dummy(client_id, display_name, transform=None, parent_handle=-1, debug=FLAGS.csimDebugMode, operation_mode=sim_code.simx_opmode_blocking):
    ...
    res, ret_ints, ret_floats, ret_strings, ret_buffer = sim_code.simxCallScriptFunction(
        client_id,
        'remoteApiCommandServer',
        sim_code.sim_scripttype_childscript,
...
def set_vision_sensor_image(client_id, display_name, image, convert=None, scale_factor=256000.0, operation_mode=sim_code.simx_opmode_oneshot_wait):
...
def drawLines(client_id, display_name, lines, parent_handle=-1, transform=None, debug=FLAGS.csimDebugMode, operation_mode=sim_code.simx_opmode_blocking):
